...
const seedJokes = async () => {
    const jokesCounter = await Jokes.count();
    if (jokesCounter > 0) {
        console.log('seed ignoré: la base contient déjà des données !');
        return;
    };

    await Jokes.bulkCreate([
        {
		...
		{
			"question": "Salut, c'est lapinou ! lapinou qui ?",
			"answer": "lapinou year !"
		},
    ])
};
